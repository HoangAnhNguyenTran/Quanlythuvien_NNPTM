    ...
    <div style={containerStyle}>
          ...
          <div style={{ height: "350px", marginTop: "20px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.trendData}>
                ...
                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fill: '#95a5a6', fontSize: 13}} />
      ...
      <div style={{ ...chartWrapperStyle, marginTop: "20px" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "15px" }}>
          ...
          <tbody>
            {data.topBooks && data.topBooks.map((book, index) => (
              <tr key={index} style={{ borderBottom: "1px solid #f9f9f9" }}>
                <td style={{ padding: "12px", fontSize: "14px" }}>{book.ten_sach}</td>
...
function StatCard({ title, value, gradient }) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: "14px", opacity: 0.85, marginBottom: "8px", fontWeight: "500" }}>{title}</div>
...
const chartWrapperStyle = { background: "#fff", padding: "25px", borderRadius: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" };
