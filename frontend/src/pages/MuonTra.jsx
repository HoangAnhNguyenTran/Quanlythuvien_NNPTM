...
function MuonTra() {
    ...
    useEffect(() => {
        if (loanData.id_doc_gia.length >= 4) {
            axios.get(`http://localhost:5000/api/muontra/find-reader/${loanData.id_doc_gia}`)
                .then(res => {
                ...
                .catch(() => setReaderInfo({ name: "❌ Không tìm thấy", status: "", isExpired: false }));
        } else {
    ...
    useEffect(() => {
        if (loanData.ma_vach_id.length >= 4) {
            axios.get(`http://localhost:5000/api/muontra/find-book/${loanData.ma_vach_id}`)
                .then(res => setBookInfo({ name: res.data.ten_sach, status: res.data.trang_thai }))
            ...
            <div style={formCard}>
                ...
                <div style={inputGroup}>
                    <div style={inputWrapper}>
                        <input 
                            style={{...inputStyle, borderColor: readerInfo.isExpired ? "red" : "#e0e7ff"}} 
                            placeholder="Mã Độc giả" 
                            value={loanData.id_doc_gia} 
                            onChange={e => setLoanData({ ...loanData, id_doc_gia: e.target.value })} 
                        />
                        <div style={{...hintStyle, color: readerInfo.isExpired ? "red" : "#5d78ff"}}>
                            {readerInfo.name} {readerInfo.status}
                        </div>
                    </div>
                    <div style={inputWrapper}>
                        <input style={inputStyle} placeholder="Mã vạch sách" value={loanData.ma_vach_id} 
                            onChange={e => setLoanData({ ...loanData, ma_vach_id: e.target.value })} />
                        <div style={hintStyle}>{bookInfo.name}</div>
                    </div>
                    <div style={inputWrapper}>
                        <input type="date" style={inputStyle} value={loanData.han_tra} 
                            onChange={e => setLoanData({ ...loanData, han_tra: e.target.value })} />
                        <div style={hintStyle}>Hạn trả</div>
                    </div>
                    <button 
                        onClick={handleLoan} 
                        disabled={readerInfo.isExpired}
                        style={{...primaryBtn, opacity: readerInfo.isExpired ? 0.5 : 1, cursor: readerInfo.isExpired ? "not-allowed" : "pointer"}}
                    >
                        XÁC NHẬN MƯỢN
                    </button>
                </div>
...
const primaryBtn = { background: "#2ecc71", color: "#fff", border: "none", padding: "12px 25px", borderRadius: "12px", fontWeight: "700" };
