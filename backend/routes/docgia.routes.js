...
router.get("/", async (req, res) => {
    try {
        const [rows] = await db.query("SELECT * FROM docgia ORDER BY id_doc_gia DESC");
        res.json(rows);
    } catch (err) {
...
router.post("/", async (req, res) => {
    const { id_doc_gia, ho_ten, email, loai_doc_gia, ngay_het_han_the } = req.body;
    
    // Kiểm tra dữ liệu bắt buộc
    if (!id_doc_gia || !ho_ten || !ngay_het_han_the) {
        return res.status(400).json({ error: "Vui lòng nhập đầy đủ Mã ĐG, Họ tên và Ngày hết hạn!" });
    }
...
router.delete("/:id", async (req, res) => {
    try {
        await db.query("DELETE FROM docgia WHERE id_doc_gia = ?", [req.params.id]);
        res.json({ success: true, message: "Đã xóa độc giả!" });
    } catch (err) {
        // Lỗi này thường xảy ra do ràng buộc khóa ngoại với bảng phieumuon
        res.status(500).json({ error: "Không thể xóa độc giả này (đã có lịch sử mượn sách)!" });
    }
});
