...
router.get("/summary", async (req, res) => {
    try {
        ...
        const [borrowing] = await db.query("SELECT COUNT(*) as total FROM chitietphieumuon WHERE ngay_tra_thuc_te IS NULL");
        
        // Lấy tổng tiền phạt thực tế từ DB
        const [fines] = await db.query("SELECT SUM(COALESCE(tien_phat_tre, 0)) as total FROM chitietphieumuon");
